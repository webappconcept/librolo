...
export default function PageManager({
  ...
  function handleNewChild(parentId: number) {
    ...
    if (!parent) {
      router.push(`/admin/pages/new?parentId=${parentId}`);
      return;
    }
    ...
    if (allowedIds.length === 1) {
      router.push(
        `/admin/pages/new?parentId=${parentId}&templateId=${allowedIds[0]}&templateLocked=1`,
      );
      return;
    }
  ...
  function handlePickerSelect(templateId: number) {
    ...
    router.push(
      `/admin/pages/new?parentId=${pickerParent.id}&templateId=${templateId}&templateLocked=1`,
    );
  }
        ...
        {target.descendants > 0 && (
          <>
            ...
            <span
              ...
              {target.descendants === 1 ? "child page" : "child pages"}.
            </span>
          </>
        )}
        <br />
      ...
      {rootPages.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p
            ...
            style={{ color: "var(--admin-text-muted)" }}>
            {searchActive ? "No pages found" : "No pages yet"}
          </p>
          <p
            className="text-xs mt-1"
            style={{ color: "var(--admin-text-faint)" }}>
            {!searchActive && 'Click "New page" to get started.'}
          </p>
        </div>
      ) : (
