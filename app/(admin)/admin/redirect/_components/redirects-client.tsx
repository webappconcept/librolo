...
const STATUS_COLORS: Record<number, { bg: string; text: string; border: string }> = {
  301: { bg: "color-mix(in srgb, #22c55e 10%, var(--admin-card-bg))", text: "#22c55e", border: "color-mix(in srgb, #22c55e 25%, transparent)" },
  302: { bg: "color-mix(in srgb, #3b82f6 10%, var(--admin-card-bg))", text: "#3b82f6", border: "color-mix(in srgb, #3b82f6 25%, transparent)" },
  307: { bg: "color-mix(in srgb, #8b5cf6 10%, var(--admin-card-bg))", text: "#8b5cf6", border: "color-mix(in srgb, #8b5cf6 25%, transparent)" },
  308: { bg: "color-mix(in srgb, #f59e0b 10%, var(--admin-card-bg))", text: "#f59e0b", border: "color-mix(in srgb, #f59e0b 25%, transparent)" },
};
...
export default function RedirectsClient({ rows: initialRows, upsertAction, deleteAction }: Props) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: "color-mix(in srgb, var(--admin-accent) 12%, var(--admin-card-bg))", border: "1px solid color-mix(in srgb, var(--admin-accent) 25%, transparent)" }}
          >
            <GitMerge size={18} style={{ color: "var(--admin-accent)" }} />
          </div>
          <div>
            <h1 className="text-lg font-semibold" style={{ color: "var(--admin-text)" }}>Redirect</h1>
            <p className="text-xs" style={{ color: "var(--admin-text-faint)" }}>{rows.length} redirect configurati</p>
          </div>
        </div>
        {mode === null && (
          <button
          ...
          >
            <Plus size={15} /> Aggiungi redirect
          </button>
        )}
      </div>
        ...
        >
          ...
          <form action={formAction} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label style={labelStyle}>Path sorgente (from)</label>
                <input
                  name="fromPath"
                  ...
                  style={inputStyle}
                />
                <p style={{ fontSize: "0.7rem", color: "var(--admin-text-faint)", marginTop: "0.25rem" }}>Deve iniziare con /</p>
              </div>
              <div>
                <label style={labelStyle}>Path destinazione (to)</label>
                <input
                  name="toPath"
            ...
            <div className="max-w-xs">
              <label style={labelStyle}>Codice HTTP</label>
              <select
                name="statusCode"
                defaultValue={String(editRow?.statusCode ?? "301")}
                style={{ ...inputStyle, fontFamily: "inherit" }}
              >
                <option value="301">301 — Permanente (SEO safe)</option>
                <option value="302">302 — Temporaneo</option>
                <option value="307">307 — Temporaneo (preserva metodo)</option>
                <option value="308">308 — Permanente (preserva metodo)</option>
              </select>
            </div>
      ...
      {rows.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm font-medium" style={{ color: "var(--admin-text-muted)" }}>Nessun redirect configurato</p>
          <p className="text-xs mt-1" style={{ color: "var(--admin-text-faint)" }}>I redirect 301 automatici vengono creati quando cambi lo slug di una pagina.</p>
        </div>
      ) : (
        <div
          ...
          {rows.map((row, i) => (
            ...
            >
              <code
                ...
                style={{ color: row.isActive ? "var(--admin-text)" : "var(--admin-text-faint)" }}
              >
                {row.fromPath}
              </code>
